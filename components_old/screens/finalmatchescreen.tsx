...
export default function FinalMatchScreen({ movie, onReset, onBack, onShare, setMatchPopup, matchPopup, finalMatch }: Props) {
  return (
    <div style={styles.screen}>
      <div style={styles.header}>
        <button style={styles.headerBtn} onClick={onBack}>←</button>
        <span style={styles.headerTitle}>Match finale</span>
        <button style={styles.headerBtn} onClick={onShare}>↗</button>
      </div>
      <div style={styles.matchFullWrapper}>
        <div style={styles.matchBadge}>🎉 MATCH!</div>
        <img
          src={movie.cover?.startsWith('http') ? movie.cover : 'https://via.placeholder.com/300x420'}
          style={styles.matchImageLarge}
        />
        <div style={styles.matchTitleLarge}>{movie.title}</div>
        <div style={styles.matchMeta}>{movie.year} • {movie.genre}</div>
        <button style={styles.watchBtn} onClick={() => movie.trailer && window.open(movie.trailer, '_blank')}>
          Guarda trailer
        </button>
        <button style={styles.resetBtn} onClick={onReset}>↻ Nuova scelta</button>
      </div>
      {matchPopup && (
        <div style={styles.popupOverlay}>
          <div style={styles.popupCard}>
            ...
            <button style={styles.popupClose} onClick={() => setMatchPopup(null)}>Chiudi</button>
          </div>
        </div>
      )}
    </div>
