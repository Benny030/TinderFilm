...
export default async function handler(
  ...
  const usernameResult = moderateText(username, 'username');

  if (!usernameResult.allowed) {
    return res.status(400).json({
      success: false,
      error: moderationMessage(usernameResult, 'username'),
    });
  }

  const bioResult = moderateText(bio, 'bio');

  if (!bioResult.allowed) {
    return res.status(400).json({
      success: false,
      error: moderationMessage(bioResult, 'bio'),
    });
  }
  ...
  if (genres.length < 3) {
    return res.status(400).json({
      success: false,
      error: 'Scegli almeno 3 generi cinematografici.',
    });
  }
