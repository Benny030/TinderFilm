...
import { crumbleCurrentSwipeCard } from '@/utils/rooms/crumbleCurrentSwipeCard';
...
export default function StanzaPage({ movies: initialMovies, roomId }: Props) {
  ...
  /*
   * Film diventati match del gruppo prima che questo client
   * riuscisse a votarli. Non sono like/pass: vengono semplicemente
   * esclusi dal deck locale dopo l'animazione di sgretolamento.
   */
  const [invalidatedMovieIds, setInvalidatedMovieIds] =
    useState<Set<string>>(() => new Set());

  /*
   * Impedisce di votare la card successiva mentre stiamo aspettando
   * la risposta server relativa allo swipe appena concluso.
   * Evita race e doppi swipe durante la gestione MOVIE_ALREADY_MATCHED.
   */
  const [swipeResolving, setSwipeResolving] = useState(false);
...
useEffect(() => {
  ...
  const remaining = movies.filter(
    (m) =>
      swipes[m.id]?.[userId] === undefined &&
      !invalidatedMovieIds.has(String(m.id))
  );
  ...
  const {
    ...
    if (currentMovie && !swipeResolving) {
      void handleSwipe(currentMovie.id, liked);
    }
  });
  ...
  const [roomCity, setRoomCity] =
    useState<string | null>(null);

  const [selectedCinemaName, setSelectedCinemaName] =
    useState<string | null>(null);

  const [selectedShowtimeAt, setSelectedShowtimeAt] =
    useState<string | null>(null);

  const [selectedBookingUrl, setSelectedBookingUrl] =
    useState<string | null>(null);
  ...
  const [isRoomLocked, setIsRoomLocked] =
    useState(false);

  const [hostActionError, setHostActionError] =
    useState('');
  ...
  const [
    membershipStatus,
    setMembershipStatus,
  ] = useState<'pending' | 'active'>('active');
  ...
  const rebuildStateFromServer = async () => {
    ...
    const [
      ...
      fetch(
        `/api/rooms?id=${encodeURIComponent(roomId)}`
      ),
      fetch(
        `/api/room-participants?roomId=${encodeURIComponent(roomId)}`
      ),
      fetch(
        `/api/swipes?roomId=${encodeURIComponent(roomId)}`
      ),
    ]);
    ...
    if (roomRes.ok) {
      ...
      setHostActorId(
        typeof room.host_actor_id === 'string'
          ? room.host_actor_id
          : null
      );
      ...
      setSelectedMovieId(
        typeof room.selected_movie_id === 'string'
          ? room.selected_movie_id
          : null
      );

      setRoomCity(
        typeof room.city === 'string'
          ? room.city
          : null
      );

      setSelectedCinemaName(
        typeof room.selected_cinema_name === 'string'
          ? room.selected_cinema_name
          : null
      );

      setSelectedShowtimeAt(
        typeof room.selected_showtime_at === 'string'
          ? room.selected_showtime_at
          : null
      );

      setSelectedBookingUrl(
        typeof room.selected_booking_url === 'string'
          ? room.selected_booking_url
          : null
      );

      setIsRoomLocked(
        Boolean(room.is_locked)
      );
      ...
      if (resolvedHostId === userId) {
        const pendingRes = await fetch(
          `/api/room-participants?roomId=${encodeURIComponent(
            roomId
          )}&pending=1&requesterId=${encodeURIComponent(
            userId
          )}`
        );
  ...
  useEffect(() => {
    ...
    const sendHeartbeat = async () => {
      ...
      try {
        ...
        // Il prossimo heartbeat ritenterà.
      }
    };
  ...
  useEffect(() => {
      ...
      async (event) => {
        ...
        if (
          ...
          const {
            ...
            .from('rooms')
            .select(
              'room_type, selected_movie_id, room_phase'
            )
            .eq('id', roomId)
            .maybeSingle();
    ...
    channel.on(
      ...
      (event) => {
        ...
        /*
         * MATCH GLOBALE:
         * questo comportamento resta invariato.
         */
        setScreen('match');
      }
    );
  ...
  // Durante waiting il DB è la fonte di verità.
  useEffect(() => {
    if (
      !roomId ||
      roomPhase !== 'waiting'
  ...
  async function handleSwipe(
    ...
    setSwipeResolving(true);

    /*
     * Manteniamo lo swipe ottimistico:
     * normalmente la UX resta identica.
     */
    setSwipes((prev) => ({
      ...prev,
      [key]: {
    ...
    try {
      ...
      if (!response.ok) {
        if (
          response.status === 409 &&
          data?.code ===
            'MOVIE_ALREADY_MATCHED' &&
          data?.match?.movie_id
        ) {
          const matchedMovieId =
            String(
              data.match.movie_id
            );

          /*
           * Questo voto era ottimistico ma il server NON lo ha accettato.
           * Lo rimuoviamo prima di rimettere la vecchia card nel deck.
           */
          setSwipes((prev) => {
            const movieVotes = {
              ...(
          ...
          /*
           * Il match globale è già avvenuto.
           * Questo utente NON deve ricevere un MatchScreen personale.
           *
           * Aspettiamo che React ripristini la card e la facciamo
           * sgretolare. Solo al termine la escludiamo dal deck locale.
           */
          await crumbleCurrentSwipeCard();
  ...
  const handleReset = async () => {
    if (
      !window.confirm(
        'Ricominciare da capo? Tutti gli swipe della stanza verranno azzerati.'
      )
    ) {
      return;
    }
        ...
        {screen === 'welcome' && (
          <WelcomeRoom
            ...
            onFinishRoom={() => {
              if (
                window.confirm(
                  'Chiudere definitivamente questa stanza?'
                )
              ) {
                void runHostAction(
                  'finish_room'
...
export const getServerSideProps:
  ...
  async ({ query }) => {
    ...
    let mode =
      (query.mode as string | undefined) ?? null;

    let genres =
      (query.genres as string | undefined) ?? null;

    let yearFrom =
      (query.year_from as string | undefined) ?? null;

    let yearTo =
      (query.year_to as string | undefined) ?? null;
