    ...
    useEffect(() => {
        // ─── Prendi coordinate salvate o chiedi geolocalizzazione ────────────────
        const check = async () => {
        setLoading(true);
        ...
        // ─── Prova prima coordinate in sessionStorage (se già ottenute) ────────
        ...
        // ─── Altrimenti chiedi geolocalizzazione ──────────────────────────────
        if (lat === null && navigator.geolocation) {
            ...
            sessionStorage.setItem('cineDateUserCoords', JSON.stringify({ lat, lng }));
            } catch { /* utente ha rifiutato */ }
        }
    ...
    // ─── Nessun cinema trovato — non mostrare niente ──────────────────────────
    ...
    return (
        ...
        {loading ? (
            <div style={{
            padding: S.md, background: C.bgSoft, borderRadius: R.md,
            display: 'flex', alignItems: 'center', gap: S.sm,
            fontSize: TEXT.sm, color: C.muted,
            }}>
            <CircleNotch size={16} color={C.muted} style={{ animation: 'spin 1s linear infinite' }} />
            ...
            {showings.map((s) => (
                ...
                >
                {/* Cinema info */}
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: S.sm }}>
                    <div>
                    <div style={{ fontSize: TEXT.sm, fontWeight: '700', color: C.ink }}>
                        🎬 {s.cinema}
                    </div>
                    ...
                    >
                    <Ticket size={13} color="#fff" weight="fill" />
                    Biglietti
                    </a>
                </div>
